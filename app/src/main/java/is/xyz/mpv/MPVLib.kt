...
object MPVLib {
    ...
    private val libraryLoadFailure: Throwable? = runCatching {
    ...
    init {
        libraryLoadFailure?.let { error ->
            Log.e(Tag, "failed to load libmpv runtime", error)
        }
    }

    val isAvailable: Boolean
        get() = libraryLoadFailure == null

    val loadErrorMessage: String?
        get() = libraryLoadFailure?.message

    fun requireAvailable() {
        check(isAvailable) {
            "libmpv runtime unavailable${loadErrorMessage?.let { ": $it" } ?: ""}"
        }
    }
